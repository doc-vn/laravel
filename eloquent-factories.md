...
Ngoài ra, bạn có thể ghi đè lên phương thức `newFactory` này trên model của bạn để trả về trực tiếp một instance của factory tương ứng của model:
...
Trong một sequence closure, bạn có thể truy cập vào thuộc tính `$index` trên instance sequence được khai báo trong closure. Thuộc tính `$index` sẽ chứa số lần lặp chuỗi:
