...
> [!NOTE]
> Thỉnh thoảng, bạn có thể muốn chỉ định các giá trị mặc định cho các tham số URL trên toàn bộ request, chẳng hạn như ngôn ngữ hiện tại. Để thực hiện điều này, bạn có thể sử dụng phương thức [URL::defaults](/docs/{{version}}/urls#default-values).
...
Ngoài việc giới hạn tỷ lệ cho các request gửi đến, Laravel còn cho phép bạn giới hạn tỷ lệ dựa trên response bằng phương thức `after`. Điều này rất hữu ích khi bạn chỉ muốn count một số response nhất định vào giới hạn tỷ lệ, ví dụ như lỗi validation, response 404, hoặc các HTTP status code cụ thể khác.

Phương thức `after` chấp nhận một closure nhận vào response và trả về `true` nếu response đó được tính vào giới hạn tỷ lệ, hoặc `false` nếu nó nên được bỏ qua. Điều này đặc biệt hữu ích để ngăn chặn các cuộc tấn công quét thông tin bằng cách giới hạn các response 404 liên tiếp, hoặc cho phép người dùng thử lại các request thất bại khi validation mà không làm ảnh hưởng đến giới hạn tỷ lệ của họ trên một endpoint mà chỉ nên giới hạn các hành động thành công:
...
Bạn có thể tham khảo tài liệu API cho [class facade Route](https://api.laravel.com/docs/{{version}}/Illuminate/Routing/Router.html) và cả [instance Route](https://api.laravel.com/docs/{{version}}/Illuminate/Routing/Route.html) để biết tất cả các phương thức có sẵn trên facade route và các class route.
