...
Tuy nhiên, để thực hiện được điều này, bạn cần phải cho Vite biết về các asset của bạn bằng cách import các asset đó vào một file entry point của ứng dụng. Ví dụ: nếu bạn muốn xử lý và tạo version cho tất cả hình ảnh được lưu trong `resources/images` và tất cả các font chữ được lưu trong `resources/fonts`, bạn nên thêm nội dung sau vào file `resources/js/app.js` của ứng dụng:
