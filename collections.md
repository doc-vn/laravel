...
Bạn cũng có thể tạo một collection bằng cách sử dụng các phương thức [make](#method-make) và [fromJson](#method-fromjson).
...
Phương thức `collapse` sẽ thu gọn một tập hợp các mảng hoặc collection con thành một collection chung và ngang hàng với nhau:
...
Phương thức `collapseWithKeys` sẽ làm phẳng hàng một collection các mảng hoặc collection thành một collection duy nhất và giữ nguyên các khóa. Nếu collection đã phẳng, nó sẽ trả về một collection rỗng:
...
Phương thức này có cùng chức năng với phương thức [contains](#method-contains); tuy nhiên, tất cả các giá trị được so sánh đều sử dụng phép so sánh "nghiêm ngặt".
...
Bạn có thể truyền vào một closure cho phương thức `countBy`, để nó sẽ đếm tất cả các item theo quy luật tùy chỉnh của bạn:
...
Phương thức này có cùng chức năng với phương thức [doesntContain](#method-doesntcontain); tuy nhiên, tất cả các giá trị được so sánh đều sử dụng phép so sánh "nghiêm ngặt".
...
Phương thức static `fromJson` sẽ tạo một instance collection mới bằng cách giải mã một chuỗi JSON đã cho bằng hàm PHP `json_decode`:
...
> [!NOTE]
> Hành vi của phương thức này sẽ được thay đổi khi tương tác với [Eloquent collection](/docs/{{version}}/eloquent-collections#method-partition).
...
Phương thức `pop` sẽ xóa và trả về item cuối cùng từ collection. Nếu collection rỗng, giá trị `null` sẽ được trả về:
...
Bạn cũng có thể cung cấp nhiều item cùng một lúc để nối lại vào cuối collection:
...
The `replaceRecursive` method behaves similarly to `replace`, nhưng nó sẽ đệ quy vào trong các mảng và áp dụng quy trình replace tương tự cho các giá trị ở bên trong:
...
Phương thức `toPrettyJson` sẽ chuyển một collection thành một chuỗi định dạng JSON bằng cách sử dụng tùy chọn `JSON_PRETTY_PRINT`:
...
Phương thức `unless` sẽ chạy hàm callback đã cho nếu như tham số đầu tiên được cung cấp cho phương thức này là khác `true`. Instance collection và tham số được truyền vào cho phương thức `unless` cũng sẽ được cung cấp cho closure:
...
Phương thức `where` sẽ sử dụng phép so sánh "lỏng lẻo" khi kiểm tra các giá trị của item, nghĩa là một chuỗi có giá trị integer sẽ bằng với một số integer có cùng giá trị. Sử dụng phương thức [whereStrict](#method-wherestrict) để lọc collection bằng các so sánh "nghiêm ngặt", hoặc các phương thức [whereNull](#method-wherenull) và [whereNotNull](#method-wherenotnull) để lọc các giá trị `null`.
...
Phương thức `withHeartbeat` cho phép bạn chạy một callback tại các khoảng thời gian nhất định trong khi một lazy collection đang được lặp. Điều này đặc biệt hữu ích cho các tác vụ chạy lâu dài yêu cầu các công việc bảo trì định kỳ, chẳng hạn như gia hạn lock hoặc update progress:
