...
**Ngay cả khi bạn chọn không sử dụng starter kit trong ứng dụng Laravel của bạn, việc cài đặt [starter kit](/docs/{{version}}/starter-kits) có thể là một cơ hội tuyệt vời để bạn tìm hiểu về cách triển khai tất cả chức năng xác thực của Laravel trong một dự án Laravel mới.** Vì Laravel starter kit sẽ tạo các controller, route và view xác thực cho bạn, bạn có thể kiểm tra code trong các file này và tìm hiểu về cách triển khai các tính năng xác thực của Laravel.
...
Như đã thảo luận ở trên, bạn có thể tương tác với các service xác thực theo cách thủ công để xây dựng lớp xác thực riêng cho ứng dụng của bạn. Tuy nhiên, để giúp bạn bắt đầu nhanh hơn, chúng tôi đã phát triển [free starter kits](/docs/{{version}}/starter-kits) cung cấp khung sườn mạnh mẽ, hiện đại cho toàn bộ lớp xác thực cho người dùng.
...
Và, nếu bạn muốn bắt đầu nhanh, chúng tôi rất vui lòng giới thiệu [starter kit của Laravel](/docs/{{version}}/starter-kits) như một cách nhanh chóng để bắt đầu một ứng dụng Laravel mới, nó sử dụng một loạt các xác thực ưa thích của chúng tôi trong các service Laravel xác thực được tích hợp sẵn và Laravel Sanctum.
...
Trước tiên, bạn nên [cài đặt bộ khởi động ứng dụng Laravel](/docs/{{version}}/starter-kits). Bộ công cụ khởi động của chúng tôi sẽ cung cấp các điểm khởi đầu được tốt, đẹp mắt để kết hợp phần xác thực này vào trong ứng dụng Laravel mới của bạn.

<a name="retrieving-the-authenticated-user"></a>
### Lấy user đã được authenticate

Sau khi tạo một ứng dụng từ starter kit và cho phép người dùng đăng ký và xác thực với ứng dụng của bạn, bạn sẽ thường xuyên phải tương tác với những người dùng đã được xác thực. Khi xử lý một request đến, bạn có thể truy cập vào người dùng đã được authenticate thông qua facade `Auth`:
...
Khi middleware `guest` phát hiện ra một người dùng đã được xác thực, nó sẽ redirect người dùng đó đến route có tên là `dashboard` hoặc `home`. Bạn có thể thay đổi hành động này bằng cách sử dụng phương thức `redirectUsersTo` trong file `bootstrap/app.php` của ứng dụng:
...
Bạn có thể sử dụng phương thức `once` để xác thực người dùng vào application cho một request. Không có session hoặc cookie nào được sử dụng khi gọi phương thức này, và event `Login` sẽ không được gửi đi:
...
Nếu bạn đang sử dụng [PHP FastCGI](https://www.php.net/manual/en/install.fpm.php) và Apache để làm ứng dụng Laravel của bạn, thì xác thực HTTP Basic có thể không hoạt động chính xác. Để khắc phục sự cố này, các dòng sau có thể được thêm vào file `.htaccess` trong ứng dụng của bạn:
