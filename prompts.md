...
Hàm `notify` sẽ gửi một thông báo hiển thị trên màn hình máy tính từ terminal:
...
Thông báo sẽ được hỗ trợ trên macOS (thông qua `osascript`) và Linux (thông qua `notify-send` cùng với `kdialog` dự phòng).

Trên macOS, bạn cũng có thể thêm một `subtitle` và một `sound`:
...
Trên Linux, bạn có thể cung cấp một `icon` tùy biến:
