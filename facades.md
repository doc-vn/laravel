...
Trong suốt tài liệu của Laravel, nhiều ví dụ sẽ sử dụng các facade để thể hiện các tính năng khác nhau của framework.
...
Hãy lưu ý rằng, ở đầu file chúng ta đã "importing" một facade `Cache`. Facade này đóng vai trò như một proxy để truy cập vào implementation của interface `Illuminate\Contracts\Cache\Factory`. Bất kỳ hàm gọi nào mà chúng ta thực hiện bằng cách sử dụng facade sẽ được chuyển đến instance của cache service của Laravel.

Nếu chúng ta nhìn vào class `Illuminate\Support\Facades\Cache`, bạn sẽ thấy rằng không có phương thức static` get` nào cả:
