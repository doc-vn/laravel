...
Ngôn ngữ mặc định cho application của bạn được lưu trữ trong tùy chọn cấu hình `locale` của file cấu hình `config/app.php`, điều này thường được thiết lập thông qua cách sử dụng biến môi trường `APP_LOCALE`. Bạn hãy thoải mái sửa giá trị này cho phù hợp với nhu cầu application của bạn.
