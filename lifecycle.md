...
HTTP kernel cũng chịu trách nhiệm chuyển request qua middleware stack của ứng dụng. Các middleware này sẽ xử lý việc đọc ghi [HTTP session](/docs/{{version}}/session), kiểm tra nếu application đang ở trong chế độ maintenance, [kiểm tra CSRF token](/docs/{{version}}/csrf), và hơn thế nữa. Chúng ta sẽ nói thêm về những điều này sớm.
