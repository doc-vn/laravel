...
Nếu bạn muốn một số thuộc tính bị ẩn được hiển thị trên một instance model, bạn có thể sử dụng phương thức `makeVisible` hoặc phương thức `mergeVisible`. Phương thức `makeVisible` sẽ trả về instance của model:
...
Tương tự, nếu bạn muốn ẩn một số thuộc tính thường được hiển thị, bạn có thể sử dụng phương thức `makeHidden` hoặc `mergeHidden`.
...
Trong lúc chạy, bạn cũng có thể chỉ dẫn instance model thêm các thuộc tính bổ sung bằng cách sử dụng phương thức `append` hoặc phương thức `mergeAppends`. Hoặc, bạn có thể sử dụng phương thức `setAppends` để ghi đè toàn bộ mảng thuộc tính sẽ được thêm vào một instance model đã cho:
